# Script PowerShell pour exÃ©cuter le pipeline rÃ©el end-to-end
...
Write-Host "Pipeline RÃ©el - ExÃ©cution End-to-End" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
function Test-PythonVersion {
    try {
        ...
        if ($pythonVersion -match "Python 3\.10\.9") {
            Write-Host "âœ… Python 3.10.9 found: $pythonVersion" -ForegroundColor Green
            return $true
        } else {
            Write-Host "âš ï¸  Python version: $pythonVersion (expected 3.10.9)" -ForegroundColor Yellow
...
function Start-Service {
    ...
    if (-not (Test-PortAvailable -Port $Port)) {
        Write-Host "âš ï¸  Port $Port is already in use. Assuming service is already running." -ForegroundColor Yellow
        return $null
    }
...
try {
    ...
    Write-Host "ðŸ“¤ Sending request to Backend MCP..." -ForegroundColor Yellow
    
    $startTime = Get-Date
    $response = Invoke-RestMethod -Uri $url -Method Post -Body $payload -ContentType "application/json" -TimeoutSec 300
    $elapsedTime = (Get-Date) - $startTime
    
    Write-Host "âœ… Pipeline completed in $($elapsedTime.TotalSeconds.ToString('F2'))s" -ForegroundColor Green
    Write-Host ""
    ...
    if ($response.status -eq "success") {
        ...
        if ($extractedData) {
            ...
            Write-Host "âš ï¸  No extracted data found in response" -ForegroundColor Yellow
        }
    } else {
